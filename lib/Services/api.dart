...
Future<String> completeNewTest(List<testQuestions> questions) async {
  ...
  for (int i = 0; i < questions.length; i++) {
    encodedQuestions.add(toJson(questions[i]));
  }
  print(encodedQuestions);
  print(questions[0].testId);
  final response = await http.post(
    Uri.parse(postsURL + "api/tests/completeNewTest"),
    headers: <String, String>{'Content-Type': 'application/json'},
    body: jsonEncode(
        {'questionList': encodedQuestions, 'testId': questions[0].testId}),
  );
  if (response.statusCode == 200) {
    // If the server did return a 201 CREATED response,
    // then parse the JSON.
    return "Successfull Completion of Tests";
  } else {
    // If the server did not return a 201 CREATED response,
    // then throw an exception.
    throw Exception('Failed to Complete Questions');
  }
}
